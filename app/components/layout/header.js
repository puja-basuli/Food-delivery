...
export default function Header() {
  ...
  return (
    <header className="flex items-center justify-between px-6 py-4 shadow-md relative">
     <Link
  href="/"
  className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-blue-500 to-white bg-clip-text text-transparent"
>
  FROZENTREATS
</Link>
          ...
          {status !== 'authenticated' && (
            <>
              <Link
                ...
                className="px-6 py-2 rounded-full text-gray-200 font-semibold"
              >
                Login
              </Link>
              <Link
